...
// DAO(Data Access Object) - dbms(데이터베이스 연동)
const userDAO = {
  // item - 클라이언트 요청 데이터..
  ...
  signup: async (item, callback) => {
    ...
    try {
      ...
      if (respCheck[0]) {
        ...
        // 유저 password 는 hash 문자열로 변형시켜서 저장..
        const salt = await bcypt.genSalt();
        bcypt.hash(item.password, salt, async (error, hash) => {
